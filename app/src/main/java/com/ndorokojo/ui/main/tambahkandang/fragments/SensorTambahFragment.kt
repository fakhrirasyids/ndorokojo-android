...
//    }
    ...
    override fun onCreateView(
        ...
        binding.btnSave.text = StringBuilder("Simpan")
    ...
    private fun setSensorSpinner() {
        binding.edStatus.apply {
            ...
            setOnItemClickListener { _, _, position, _ ->
                if (position == 0) {
                    storeKandangViewModel?.isSensorStatusTerpasang?.postValue(true)
                } else {
                    storeKandangViewModel?.isSensorStatusTerpasang?.postValue(false)
                }
            }
        }
    }
            ...
            btnBack.setOnClickListener {
                ...
                with(builder)
                {
                    ...
                    setPositiveButton("Ya") { dialog, _ ->
                        dialog.dismiss()
                        BottomSheetTambahKandang.changeFragmentToIndex(0)
                    }
...
//                            sensorLon.postValue(if (binding.edLongitude.text.isNullOrEmpty()) null else binding.edLongitude.text.toString())
                    ...
                    with(builder)
                    {
                        ...
                        setPositiveButton("Benar") { dialog, _ ->
                            ...
                            storeKandangViewModel?.storeTernak()
                                ?.observe(viewLifecycleOwner) { result ->
                                    when (result) {
                                        ...
                                        is Result.Success -> {
                                            ...
                                            with(builder)
                                            {
                                                setTitle("Berhasil Tambah Kandang!")
                                                setMessage("Silahkan melanjutkan untuk mengolah data ternak")
                                                setPositiveButton("OK") { dialog, _ ->
                                                    dialog.dismiss()
                                                    bottomSheetTambahKandang?.dismiss()
                                                    bottomSheetTambahKandang = null
                                                }
                                                show()
                                            }
                                        }
